...
export interface IColumnConfig extends IColumn {
  ...
  dataType: 'boolean' | 'date' | 'number' | 'string' | 'undefined';
  ...
  /** (KEYS STILL NEED TO BE UNIQUE) if `inputType` is `EditControlType.DropDown` - Filter the options based on the key value. Will work if `inputType` is `EditControlType.ComboBox` only if 'multiselct' is enabled*/
