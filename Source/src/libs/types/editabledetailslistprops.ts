...
export interface EditableGridProps extends IDetailsListProps {
  /**
   * Will remove the ablity to delete in row actions for rows not added by the user
   * @default false */
  enableSignleRowDeletionOnAddedRowsOnly?: boolean;

  /** @default true*/
  autoDeleteBlankRows?: boolean;
  ...
  /** If Commandbar is not resisizing properly, add a custom/dynamic cache key here. The grid will make it unique to each item based on the key provided. */
  commandbarCacheKey?: string | null | undefined | number;
  ...
  enableSearchBox?: {
    enable: boolean;
    /** How the searchbox searches when the user enters a value*/
  ...
  /**Keys that do not count toward triggering a duplicate row error */
  ignoreTheseKeysWhenDeterminingDuplicatedRows?: string[];
