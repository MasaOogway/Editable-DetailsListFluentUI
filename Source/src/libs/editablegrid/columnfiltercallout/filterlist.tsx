...
import { stackTokens, styles } from '../../editablegrid/columnfiltercallout/filterliststyles';
...
const FilterList = (props: Props) => {
  const [filterItemsList, setFilterItemsList] = useState<IFilterListItem[]>([]);
  const [filterListContent, setFilterListContent] = useState<JSX.Element[] | undefined>([]);
  const [appliedFilters, setAppliedFilters] = useState<IFilterItem[]>([]);

  const [isSelectAllIndeterminate, setIsSelectAllIndeterminate] = useState(true);
      ...
      searchResult.filter((_data, index) => {
        ...
        try {
          if (_data.text.toString().toLowerCase().includes(text.trim().toLowerCase())) {
            ...
            onCheckChanged(ev, true, _data.key, _data.text);
            throw BreakException;
          } else {
            _data.isFilteredIn = false;
            _data.isChecked = false;
            onCheckChanged(ev, false, _data.key, _data.text);
          }
        } catch (e) {
    ...
    <>
      <Stack verticalAlign="start" tokens={stackTokens}>
        ...
        <Stack horizontal horizontalAlign="start">
          <ActionButton className={styles.button} onClick={onApply} text="Apply" />
          ...
          <ActionButton text="Close" className={styles.button} onClick={props.onClose} />
        </Stack>
      </Stack>
    </>
  );
